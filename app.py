...
def init_game():
    ...
    def _setup():
        ...
        try:
            ...
            data["status"]        = "ready"
            pool = data["order_pool"]
                    # available_order_ids already set in prepare_game
...
def api_state():
    ...
    # When selecting a venue, send ranked venue list to the right panel
    venue_options = []
...
def api_accept():
    ...
    # Search for venues near the PLAYER's current position
    ...
    try:
        ...
        # Chain through fallback queries until we get results
    ...
    # Sort by distance from player and add rank (so map tooltip ↔ panel card numbering match)
...
def api_select_venue():
    ...
    # Patch the order inside the pool (mutate in place)
    ...
    g["phase"]             = "pickup"
    g["active_selected_at"]= time.time()   # timer starts NOW
    g["map_html"]          = build_map(g)
    set_game(g)
...
def api_cancel_venue():
    ...
    g["active_order"]        = None
    g["phase"]               = "idle"
    # Put the order back at the front of available list
